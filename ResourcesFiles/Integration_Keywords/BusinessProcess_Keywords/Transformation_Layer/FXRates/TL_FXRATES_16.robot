...
*** Keywords ***
Send GSFile for FX Rates with Invalid Field Length
    [Documentation]    Used to send Golden Source file with invalid field length.
    ...
    ${invalidLengthResponseList}    Create List    ${INVALID_GS_INSTR_TYPE}    ${INVALID_GS_SNAP}    ${INVALIDGS_BASE_CCY}    ${INVALID_GS_TRGT_CCY}    ${INVALID_GS_INSTR_PRC_TYPE}    ${INVALID_GS_INSTR_PRICE}    ${INVALID_GS_PROCESSING_DATE}

    Run Keyword And Continue On Failure    Validate FFC for TL FXRates Failed    &{ExcelPath}[OutputFilePath]    &{ExcelPath}[OutputFFCResponse]    ${invalidLengthResponseList}    isMultipleExpectedResponse=${True}
