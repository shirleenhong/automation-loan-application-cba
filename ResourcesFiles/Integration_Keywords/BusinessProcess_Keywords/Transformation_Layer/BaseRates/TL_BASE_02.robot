...
Send GS File with Same Base Rate, Tenor and Effective Date for Grouping
    ...
    ...                                      - changed the keyword in validating FFC with multiple sub-entities
    [Arguments]    ${ExcelPath}
    ...
    Test 1    ${CSVFile}    ${TransformedDataFile_BaseRate}    ${TransformedDataFile_Template_BaseRate}
    ...
    Run Keyword And Continue On Failure    Validate FFC for TL Base Rate Success with Multiple SubEntities    &{ExcelPath}[InputFilePath]    &{ExcelPath}[InputJson]    &{ExcelPath}[Expected_wsFinalLIQDestination]
