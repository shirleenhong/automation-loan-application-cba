...
Send GS File with Overnight Rates
    [Documentation]    Used to send a valid Golden Source file with overnight rates and floating rates will be removed to SFTP site. 
    ...    Then validate if GS file is processed and moved to Archive folder. Then validate FFC if file is sent to CCB OpenAPI, 
    ...    distributor and CustomCBAPush. Then validate in LoanIQ if Base Rate. Code is updated correctly.
    ...    @author: jdelacru    06AUG2019    - initial create
    ...    @update: mcastro     05OCT2020    - added missing argument variable for Create expected TextJMS XML
