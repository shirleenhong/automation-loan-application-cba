...
*** Keywords ***
Send a SENT Callback for Rate Setting Notice for SYNDICATED Deal via WIP
    [Documentation]    This keyword is use to successfully sent out a Rate Setting Notice via WIP - API_COR_TC20
    ...    @author:mgaling
    ...
    ...    @update: ehugo    05SEP2019    added validation of response code when sending call back
    ...
    ${Loan_RepricingDate}    Read Data From Excel for API_Data    Correspondence    Loan_RepricingDate   ${rowid}
    ...
    ###Remove Maturity Date in Excel if Repricing Date is available - Repricing Date will be used in validation###
    ${RepricingDate_Length}    Get Length    ${Loan_RepricingDate.strip()}
    Run Keyword If    ${RepricingDate_Length}!=0    Write Data To Excel for API_Data    Correspondence    Loan_MaturityDate    ${rowid}    ${EMPTY}
    ...
    ...    &{APIDataSet}[Loan_GlobalOriginal]    &{APIDataSet}[Loan_RateSetting_DueDate]    ${Loan_RepricingDate}
