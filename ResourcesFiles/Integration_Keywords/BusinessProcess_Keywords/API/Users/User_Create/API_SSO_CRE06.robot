...
Create User for Multiple LOB with No Configuration
    [Documentation]    This keyword is used to create prerequisites for execution.
    ...    And send a POST request for LIQ, Essence and Party user. And validate MCH UI for AD, SSO, LIQ, Essence and Party.
    ...
    ...    @update: cfrancis    23JUL2020    - modified to index some of the pre requisite values
    ...
    ...    ${DefaultZone_Config_PARTY}    ${DefaultBranch_Config_PARTY}    &{APIDataSet}[contactNumber1]    &{APIDataSet}[email]
    ...
    ...    ${Title_Config_COREBANKING}    &{APIDataSet}[osUserId]    ${2Code_CountryCode}    ${CountryDesc}    ${UserType_Desc_COREBANKING}    &{APIDataSet}[locale]
    ...    ${LanguageDesc}    ${DefaultZone_Config_COREBANKING}    ${DefaultBranch_Config_COREBANKING}    &{APIDataSet}[contactNumber1]    &{APIDataSet}[email]
