...
*** Keywords ***
Create User Using Demographic Fields only without Contact Number 2
    [Documentation]    Create User using Demographic Fields only without contactNumber2
    ...    And send a POST request. And check the payload sent in FFC
    ...    Then validate success creation without contactNumber2 in SSO Page (AD is not yet Automatable)
