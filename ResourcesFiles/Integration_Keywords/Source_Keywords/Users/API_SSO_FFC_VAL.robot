...
Validate FFC for Multiple LOB for Success
    ...
    Run Keyword If    '${JMS_CORRELATION_ID}'!=''    Run Keyword And Continue On Failure    Validate AD Success Response in FFC for User API    ${sOutputFilePath}    ${sHTTPMethod}    ${sOSUserID}
...
Validate LIQ Error Response in FFC for Delete User API
    ...
    Go to Dashboard and Click Source API Name    ${TEXTJMS_SOURCENAME}    ${TEXTJMS_INSTANCE}
    ${aHeaderRefNameList}    Create List    ${JMS_CORRELATION_ID}    ${RESULTSTABLE_STATUS}    ${HEADER_QUEUENAME}
    ${aExpectedRefList}    Create List    ${ID}    ${ERROR}    ${QName_Error_Users}
    ${ColumnIndex}    Filter by Multiple Reference Headers and Values and Return Column Index    ${aHeaderRefNameList}    ${aExpectedRefList}
    ${ResultsRowList}    Save Message TextArea and Return Results Row List Value    ${ColumnIndex}    ${sOutputFilePath}${sActual_wsFinalLIQDestination}    ${XML}    ${HEADER_ROUTEROPERATION}    
    ...    ${HEADER_CATEGORY}    ${HEADER_OPERATION}
...
Validate FFC for Multiple LOB for Successful Delete
    ...
    \    Run Keyword If    '${LOB_Value}'=='ALL' and '${sLIQError}'=='None'    Run Keywords    Run Keyword and Continue on Failure    Validate LIQ Success Response in FFC for Delete User API    ${sInputFilePath}    ${sOutputFilePath}    ${sHTTPMethod}    ${sExpected_wsFinalLIQDestination}    ${sActual_wsFinalLIQDestination}
         ...
         ...    ELSE IF    '${LOB_Value}'=='COMRLENDING' and '${sLIQError}'=='None'    Run Keyword and Continue on Failure    Validate LIQ Success Response in FFC for Delete User API    ${sInputFilePath}    ${sOutputFilePath}    ${sHTTPMethod}    ${sExpected_wsFinalLIQDestination}    ${sActual_wsFinalLIQDestination}    
