...
GET Request for User API Single Lob
    ...
    ${InputValue}    Set Variable    ${sLoginId}${LOBS}${sLOB}/dataSource/LOB
...
Request HTTP Method for User API for All User Endpoints And Validate Response Code
    ...
    Run Keyword If    '${sMethod}' == 'POST' or '${sMethod}' == 'PUT'    Run Keywords    Verify Json Response Status Code    ${RESPONSECODE_405}
    ...    AND    Create File    ${dataset_path}${sOutputPath}${sOutputFile}SINGLE.json    ${API_RESPONSE.content}
    
    Run Keyword If    '${sMethod}' == 'DELETE'    Run Keywords    Verify Json Response Status Code    ${RESPONSECODE_204}
    ...    AND    Create File    ${dataset_path}${sOutputPath}${sOutputFile}SINGLE.json    ${API_RESPONSE.content}
